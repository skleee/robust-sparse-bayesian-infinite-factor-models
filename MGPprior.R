...
linearMGSP = function(X, nrun, burn, thin = 1, prop = 0.7, epsilon = 1e-3,
  ...
  delta = c(rgamma(1,ad1,bd1), rgamma(k-1,ad2,bd2))       # global shrinkage coefficients multilpliers
  ...
  for(i in 1:nrun){
    ...
    if(dump & (i %% buffer == 0)){
      ...
      if(fs) out = c(out, list(lambdaSamps = LAMBDA, etaSamps = ETA, psijhSamps = PSIJH))
